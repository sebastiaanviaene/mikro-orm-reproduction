...
export class EntityManager<Driver extends IDatabaseDriver = IDatabaseDriver> {
  ...
  protected readonly refLoader = new DataLoader(
    DataloaderUtils.getRefBatchLoadFn(this)
  );
  protected readonly colLoader = new DataLoader(
    DataloaderUtils.getColBatchLoadFn(this)
  );
  ...
  getConnection(type?: ConnectionType): ReturnType<Driver["getConnection"]> {
    return this.driver.getConnection(type) as ReturnType<
  ...
  >(entityName: EntityName<Entity>): GetRepository<Entity, Repository> {
    ...
    if (!this.repositoryMap[entityName]) {
      ...
      const RepositoryClass = this.config.getRepositoryClass(
        meta.repository
      ) as Constructor<EntityRepository<any>>;
      this.repositoryMap[entityName] = new RepositoryClass(this, entityName);
    }

    return this.repositoryMap[entityName] as unknown as GetRepository<
      Entity,
      Repository
    >;
  }
  ...
  >(
    ...
    if (options.disableIdentityMap ?? this.config.get("disableIdentityMap")) {
      ...
      const ret = await fork.find(entityName, where, {
        ...options,
    ...
    const cached = await em.tryCache<
      Entity,
      Loaded<Entity, Hint, Fields, Excludes>[]
    >(entityName, options.cache, cacheKey, options.refresh, true);

    if (cached?.data) {
      await em.entityLoader.populate<Entity>(
        entityName,
    ...
    (options as Dictionary)._populateWhere =
      options.populateWhere ?? this.config.get("populateWhere");
    options.populateWhere = await this.applyJoinedFilters(
      meta,
      { ...where } as ObjectQuery<Entity>,
      options
    );
    const results = await em.driver.find(entityName, where, {
    ...
    for (const data of results) {
      const entity = em.entityFactory.create(
        entityName,
  ...
  private getPopulateWhere<Entity extends object, Hint extends string = never>(
    ...
    options: Pick<FindOptions<Entity, Hint>, "populateWhere">
  ): {
    where: ObjectQuery<Entity>;
  ...
  addFilter<T1>(
    ...
    cond:
      | FilterQuery<T1>
      | ((args: Dictionary) => MaybePromise<FilterQuery<T1>>),
  ...
  addFilter<T1, T2>(
    ...
    cond:
      | FilterQuery<T1 | T2>
      | ((args: Dictionary) => MaybePromise<FilterQuery<T1 | T2>>),
  ...
  addFilter<T1, T2, T3>(
    name: string,
    cond:
      | FilterQuery<T1 | T2 | T3>
      | ((args: Dictionary) => MaybePromise<FilterQuery<T1 | T2 | T3>>),
  ...
  addFilter(
    ...
    cond:
      | Dictionary
      | ((args: Dictionary) => MaybePromise<FilterQuery<AnyEntity>>),
    entityName?: EntityName<AnyEntity> | EntityName<AnyEntity>[],
    enabled?: boolean
  ): void;
  ...
  protected async applyJoinedFilters<Entity extends object>(
    meta: EntityMetadata<Entity>,
    cond: ObjectQuery<Entity>,
    options:
      | FindOptions<Entity, any, any, any>
      | FindOneOptions<Entity, any, any, any>
  ): Promise<ObjectQuery<Entity>> {
    ...
    if (options.populate) {
      for (const hint of options.populate as unknown as PopulateOptions<Entity>[]) {
        ...
        const where = await this.applyFilters<Entity>(
          prop.type,
          {},
          options.filters ?? {},
          "read",
        ...
        const where2 = await this.applyJoinedFilters<Entity>(
          prop.targetMeta!,
  ...
  async applyFilters<Entity extends object>(
    ...
    const push = (source: Dictionary<FilterDef>) => {
      const activeFilters = QueryHelper.getActiveFilters(
        entityName,
        options,
        source
    ...
    const conds = [...ret, where as Dictionary].filter((c) =>
      Utils.hasObjectKeys(c)
    ) as FilterQuery<Entity>[];

    return conds.length > 1
      ? ({ $and: conds } as FilterQuery<Entity>)
      : conds[0];
  }
  ...
  ): Promise<Cursor<Entity, Hint, Fields, Excludes>> {
    const em = this.getContext(false);
    entityName = Utils.className(entityName);
  ...
  >(
    ...
    if (options.disableIdentityMap ?? this.config.get("disableIdentityMap")) {
      const em = this.getContext(false);
      const fork = em.fork({ keepTransactionContext: true });
      const ret = await fork.findOne(entityName, where, {
        ...options,
        disableIdentityMap: false,
      });
      fork.clear();
    ...
    const isOptimisticLocking =
      !Utils.isDefined(options.lockMode) ||
      options.lockMode === LockMode.OPTIMISTIC;

    if (
      entity &&
      !em.shouldRefresh(meta, entity, options) &&
      isOptimisticLocking
    ) {
    ...
    const data = await em.driver.findOne<Entity, Hint, Fields, Excludes>(
      entityName,
      where,
      {
        ctx: em.transactionContext,
    ...
    entity = em.entityFactory.create<Entity>(
      entityName,
      data as EntityData<Entity>,
      {
        merge: true,
  ...
  >(
    ...
    if (options.strict) {
      const ret = await this.find(entityName, where, {
        ...options,
        limit: 2,
      } as FindOptions<Entity, Hint, Fields, Excludes>);
      isStrictViolation = ret.length !== 1;
      entity = ret[0];
    } else {
      entity = await this.findOne<Entity, Hint, Fields, Excludes>(
        entityName,
        where,
        options
      );
   ...
   */
  async upsert<Entity extends object>(
    ...
    if (Utils.isEntity(data)) {
      ...
      if (where) {
        const exists = em.unitOfWork.getById<Entity>(
          entityName,
          where as Primary<Entity>,
          options.schema
        );
    ...
    const unique =
      (options.onConflictFields as string[]) ??
      meta.props.filter((p) => p.unique).map((p) => p.name);
    const propIndex =
      !Utils.isRawSql(unique) &&
      unique.findIndex((p) => (data as Dictionary)[p] != null);

    if (options.onConflictFields || where == null) {
      if (propIndex !== false && propIndex >= 0) {
        where = {
          [unique[propIndex]]: (data as Dictionary)[unique[propIndex]],
        } as FilterQuery<Entity>;
      } else if (meta.uniques.length > 0) {
        for (const u of meta.uniques) {
          if (
            Utils.asArray<EntityKey<Entity>>(u.properties).every(
              (p) => data![p] != null
            )
          ) {
            where = Utils.asArray<EntityKey<Entity>>(u.properties).reduce(
              (o, key) => {
                o[key] = data![key];
                return o;
              },
    ...
    if (where == null) {
      const compositeUniqueProps = meta.uniques.map((u) =>
        Utils.asArray(u.properties).join(" + ")
      ) as EntityKey<Entity>[];
      const uniqueProps = meta.primaryKeys
        .concat(...(unique as EntityKey[]))
        .concat(compositeUniqueProps);
      throw new Error(
        `Unique property value required for upsert, provide one of: ${uniqueProps.join(
          ", "
        )}`
      );
    }
    ...
    em.unitOfWork
      ...
      .mapReturnedValues(entity, data, ret.row, meta);
    const uniqueFields =
    ...
    if (
      ...
      (returning.length > 0 &&
        !(this.getPlatform().usesReturningStatement() && ret.row))
    ) {
      const where = {} as FilterQuery<Entity>;

      if (Array.isArray(uniqueFields)) {
        uniqueFields.forEach(
          (prop) => (where[prop as EntityKey] = data![prop as EntityKey])
        );
      }
   ...
   * If the entity is already present in current context, there won't be any queries - instead, the entity data will be assigned and an explicit `flush` will be required for those changes to be persisted.
   */
  async upsertMany<Entity extends object>(
    entityNameOrEntity: EntityName<Entity> | Entity[],
    data?: (EntityData<Entity> | NoInfer<Entity>)[],
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      if (options.onConflictFields || where == null) {
        if (propIndex >= 0) {
          where = {
            [unique[propIndex]]: row[unique[propIndex] as EntityKey<Entity>],
          } as FilterQuery<Entity>;
        } else if (meta.uniques.length > 0) {
          for (const u of meta.uniques) {
            if (
      ...
      if (where == null) {
        ...
        const uniqueProps = (meta.primaryKeys as string[])
          .concat(...unique)
          .concat(compositeUniqueProps);
        throw new Error(
          `Unique property value required for upsert, provide one of: ${uniqueProps.join(
    ...
    const res = await em.driver.nativeUpdateMany(
      entityName,
      allWhere,
      allData,
      {
    ...
    const uniqueFields =
      ...
      ((Utils.isPlainObject(allWhere![0])
        ? Object.keys(allWhere![0]).flatMap((key) =>
            Utils.splitPrimaryKeys(key)
          )
        : meta!.primaryKeys) as (keyof Entity)[]);
    const returning = getOnConflictReturningFields(
      meta,
    ...
    const reloadFields =
      returning.length > 0 &&
      !(this.getPlatform().usesReturningStatement() && res.rows?.length);

    if (
      options.onConflictAction === "ignore" ||
      (!res.rows?.length && loadPK.size > 0) ||
      reloadFields
    ) {
      const unique = meta.hydrateProps
        .filter((p) => !p.lazy)
        .map((p) => p.name);
      const add = new Set(
        propIndex! >= 0 ? [unique[propIndex!]] : ([] as EntityKey<Entity>[])
      );
      ...
      const data2 = await this.driver.find(meta.className, where, {
        fields: returning
          .concat(...add)
          .concat(
            ...((Array.isArray(uniqueFields) ? uniqueFields : []) as string[])
          ) as any,
        ctx: em.transactionContext,
        convertCustomTypes: true,
        connectionType: "write",
      ...
      for (const [entity, cond] of loadPK.entries()) {
        ...
        if (!row) {
          throw new Error(
            `Cannot find matching entity for condition ${JSON.stringify(cond)}`
          );
        }
      ...
      if (loadPK.size !== data2.length && Array.isArray(uniqueFields)) {
        for (let i = 0; i < allData.length; i++) {
          ...
          if (!row) {
            throw new Error(
              `Cannot find matching entity for condition ${JSON.stringify(
                cond
              )}`
            );
          }
  ...
  async transactional<T>(
    ...
    const propagateToUpperContext =
      !em.global || this.config.get("allowGlobalContext");
    ...
    return TransactionContext.create(fork, async () => {
      return fork.getConnection().transactional(
            ...
            fork.eventManager.registerSubscriber({
              afterFlush(args: FlushEventArgs) {
                args.uow
                  .getChangeSets()
                  .filter((cs) =>
                    [ChangeSetType.DELETE, ChangeSetType.DELETE_EARLY].includes(
                      cs.type
                    )
                  )
        ...
        {
          ...options,
          eventBroadcaster: new TransactionEventBroadcaster(fork, undefined, {
  ...
  async begin(
    ...
    em.transactionContext = await em.getConnection("write").begin({
      ...options,
      eventBroadcaster: new TransactionEventBroadcaster(em, undefined, {
        topLevelTransaction: !options.ctx,
      }),
    });
  }
  ...
  async commit(): Promise<void> {
    ...
    await em
      .getConnection("write")
      .commit(em.transactionContext, new TransactionEventBroadcaster(em));
  ...
  async rollback(): Promise<void> {
    ...
    await em
      .getConnection("write")
      .rollback(em.transactionContext, new TransactionEventBroadcaster(em));
    delete em.transactionContext;
  ...
  async lock<T extends object>(
    ...
    options = Utils.isPlainObject(options)
  ...
  async insert<Entity extends object>(
    ...
    if (Utils.isEntity<Entity>(data)) {
      ...
      await em.unitOfWork
        .getChangeSetPersister()
        .executeInserts([cs], { ctx: em.transactionContext, ...options });
    ...
    em.validator.validateParams(data, "insert data");
    const res = await em.driver.nativeInsert<Entity>(
      entityName,
      data as EntityData<Entity>,
      { ctx: em.transactionContext, ...options }
    );
  ...
  async insertMany<Entity extends object>(
    ...
    if (data === undefined) {
      entityName = ((entityNameOrEntities as Entity[])[0] as Dictionary)
        .constructor.name;
    ...
    if (Utils.isEntity<Entity>(data[0])) {
      ...
      await em.unitOfWork
        .getChangeSetPersister()
        .executeInserts(css, { ctx: em.transactionContext, ...options });
    ...
    const res = await em.driver.nativeInsertMany<Entity>(
      entityName,
      data as EntityData<Entity>[],
      { ctx: em.transactionContext, ...options }
    );
  ...
  map<Entity extends object>(
    ...
    Object.keys(data).forEach((k) => {
      ...
      if (
        prop &&
        prop.kind === ReferenceKind.SCALAR &&
        SCALAR_TYPES.includes(prop.runtimeType) &&
        !prop.customType &&
  ...
  merge<Entity extends object>(
    ...
    if (Utils.isEntity(entityName)) {
      return em.merge(
        (entityName as Dictionary).constructor.name,
    ...
    em.validator.validatePrimaryKey(
      data as EntityData<Entity>,
      em.metadata.get(entityName)
    );
    let entity = em.unitOfWork.tryGetById<Entity>(
      entityName,
    ...
    if (
      entity &&
      helper(entity).__managed &&
      helper(entity).__initialized &&
      !options.refresh
    ) {
      return entity;
    }
    ...
    const childMeta = em.metadata.getByDiscriminatorColumn(
      meta,
      data as EntityData<Entity>
    );

    entity = Utils.isEntity<Entity>(data)
      ? data
      : em.entityFactory.create<Entity>(
          entityName,
          data as EntityData<Entity>,
          { merge: true, ...options }
        );
  ...
  create<Entity extends object, Convert extends boolean = false>(
    entityName: EntityName<Entity>,
    data: EntityData<Entity, Convert>,
  ...
  >(
    ...
    return EntityAssigner.assign(entity, data as any, {
      em: this.getContext(),
      ...options,
    }) as any;
  }
  ...
  getReference<Entity extends object>(
    ...
    const entity = this.getEntityFactory().createReference<Entity>(
      entityName,
      id,
      { merge: true, ...options }
    );
  ...
  async count<Entity extends object, Hint extends string = never>(
    entityName: EntityName<Entity>,
    where: FilterQuery<NoInfer<Entity>> = {} as FilterQuery<Entity>,
    ...
    options.populate = (await em.preparePopulate(
      entityName,
      options as FindOptions<Entity, Hint>
    )) as any;
    em.validator.validateParams(where);
    ...
    const cached = await em.tryCache<Entity, number>(
      entityName,
      options.cache,
      cacheKey
    );
    ...
    const count = await em.driver.count<Entity, Hint>(entityName, where, {
      ctx: em.transactionContext,
      ...options,
    });
  ...
  persist<Entity extends object>(
    ...
    for (const ent of entities) {
      if (!Utils.isEntity(ent, true)) {
        ...
        const meta =
          typeof ent === "object"
            ? em.metadata.find((ent as Dictionary).constructor.name)
            : undefined;
  ...
  async tryFlush<Entity extends object>(
    ...
    const em = this.getContext();
    const flushMode =
      options.flushMode ?? em.flushMode ?? em.config.get("flushMode");
    entityName = Utils.className(entityName);
    ...
    if (
      ...
      em.getUnitOfWork().shouldAutoFlush(meta)
    ) {
      await em.flush();
    }
  }
   ...
   */
  canPopulate<Entity extends object>(
    entityName: EntityName<Entity>,
    property: string
  ): boolean {
    entityName = Utils.className(entityName);
  ...
  async populate<
    Entity extends object,
    Naked extends FromEntityType<UnboxArray<Entity>> = FromEntityType<
      UnboxArray<Entity>
    >,
    Hint extends string = never,
    Fields extends string = "*",
  ...
  > {
    ...
    await em.entityLoader.populate(
      entityName,
      arr,
      preparedPopulate,
      options as EntityLoaderOptions<Entity>
    );
  ...
  fork(options: ForkOptions = {}): this {
    ...
    fork.disableTransactions =
      ...
      this.config.get("disableTransactions");
    ...
    fork.loggerContext = Utils.merge(
      {},
      em.loggerContext,
   ...
   */
  getMetadata<Entity extends object>(
    entityName: EntityName<Entity>
  ...
  getMetadata<Entity extends object>(
    entityName?: EntityName<Entity>
  ): EntityMetadata<Entity> | MetadataStorage {
    if (entityName) {
      entityName = Utils.className(entityName);
  ...
  private checkLockRequirements(
    ...
    if (
      [LockMode.PESSIMISTIC_READ, LockMode.PESSIMISTIC_WRITE].includes(mode) &&
  ...
  >(
    ...
    const preparedPopulate = await this.preparePopulate<T>(
      meta.className,
      options
    );
    await this.entityLoader.populate(
      meta.className,
      [entity],
      preparedPopulate,
      {
  ...
  private buildFields<T extends object, P extends string>(
    ...
    return fields.reduce((ret, f) => {
      if (Utils.isPlainObject(f)) {
        Utils.keys(f).forEach((ff) =>
          ret.push(
            ...this.buildFields(f[ff]!).map(
              (field) => `${ff as string}.${field}` as never
            )
          )
        );
      } else {
  ...
  private async preparePopulate<Entity extends object>(
    entityName: string,
    options: Pick<
      FindOptions<Entity, any, any>,
      "populate" | "strategy" | "fields" | "flags"
    >,
    validate = true
    ...
    if (!options.populate && options.fields) {
      ...
      const pruneToOneRelations = (
        ...
        for (const field of fields) {
          if (
            field === PopulatePath.ALL ||
            field.startsWith(`${PopulatePath.ALL}.`)
          ) {
            ret.push(
              ...meta.props
                .filter(
                  (prop) =>
                    prop.lazy ||
                    [ReferenceKind.SCALAR, ReferenceKind.EMBEDDED].includes(
                      prop.kind
                    )
                )
                .map((prop) => prop.name)
          ...
          const inner = pruneToOneRelations(prop.targetMeta!, [
            parts.join("."),
          ]);
      ...
      options.populate = pruneToOneRelations(
        meta,
        this.buildFields(options.fields)
      ) as any;
    }
    ...
    const populate: PopulateOptions<Entity>[] =
      this.entityLoader.normalizePopulate<Entity>(
        entityName,
        options.populate as true,
        options.strategy as LoadStrategy
      );
    const invalid = populate.find(
      ({ field }) => !this.canPopulate(entityName, field)
    );
    ...
    return populate.map((field) => {
      ...
      field.strategy = all
        ...
        : ((options.strategy ?? field.strategy) as LoadStrategy);
  ...
  >(meta: EntityMetadata<T>, entity: T, options: FindOneOptions<T, P, F, E>) {
    ...
    if (options.fields) {
      autoRefresh = options.fields.some(
        (field) => !helper(entity).__loadedProperties.has(field as string)
      );
    } else {
      autoRefresh = meta.comparableProps.some((prop) => {
        ...
        return (
          !inlineEmbedded &&
          !prop.lazy &&
          !helper(entity).__loadedProperties.has(prop.name)
        );
      });
    }
  ...
  protected prepareOptions(
    ...
    if (!Utils.isEmpty(options.fields) && !Utils.isEmpty(options.exclude)) {
      throw new ValidationError(
        `Cannot combine 'fields' and 'exclude' option.`
      );
    }
  ...
  async tryCache<T extends object, R>(
    ...
    refresh?: boolean,
    merge?: boolean
  ): Promise<{ data?: R; key: string } | undefined> {
  ...
  async storeCache(
    ...
    if (config) {
      const em = this.getContext();
      const expiration = Array.isArray(config)
        ? config[1]
        : Utils.isNumber(config)
        ? config
        : undefined;
      await em.resultCache.set(
        key.key,
